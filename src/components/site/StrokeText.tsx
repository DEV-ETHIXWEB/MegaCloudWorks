...
/**
 * Type that draws itself: the outline is stroked on first, then the
 * solid faces are wiped in behind it.
 *
 * The measurement pass is the part that matters. The SVG's viewBox is
 * derived from the text's own bounding box once the webfont has loaded -
 * before that the box is the fallback font's, and the headline would jump
...
export function StrokeText({
  ...
  reverse?: boolean
  /** where the drawing sits in its box when it is narrower than the box */
  ...
  return (
    ...
    >
      <svg
        ...
        viewBox={viewBox}
        /* the drawing is scaled to fit its box, and whatever width is
           left over has to go somewhere: centred by default, but a
           headline set left needs it against the left edge or it floats
           away from the copy underneath it */
        preserveAspectRatio={`${
          align === 'left' ? 'xMin' : align === 'right' ? 'xMax' : 'xMid'
        }YMid meet`}
        aria-hidden="true"
      >
        {fillMode === 'wipe' && box && (
